...
/**
 * Create by Popeye on 9:44 AM, 11/18/2020
 */
public class DarkRealmModel extends DataModel {
    public String hash;
    ...
    public DarkRealmModel() {
    }

    public DarkRealmModel(String hash) {
        this.hash = hash;
        this.bossID = "MBS1000";
        this.bossElement = TeamUtils.genElement(Element.FIRE.getId());
    ...
    public static DarkRealmModel copyFromDBtoObject(String hash, Zone zone) {
        DarkRealmModel model = null;
        try {
            String str = (String) getModel(hash, DarkRealmModel.class, zone);
            if (str != null) {
                model = Utils.fromJson(str, DarkRealmModel.class);
                if (model != null) {
                }
            }
        } catch (DataControllerException e) {
...
//                    RankDAO.updateRankDarkrealm(zone, uid, point);
                    LeagueManager.getInstance().updateRankDarkrealm(zone, uid, (int) point);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        if(userGuildModel.gid != 0){    //có trong guid
            synchronized (allianceRankObject) {
                try {
                    ...
                    if(guildModel != null){
//                        jedis.zincrby("darkrealm_leaderboard_guild_" + hash, point, String.valueOf(guildModel.gId));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
